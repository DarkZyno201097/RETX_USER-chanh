...
export default function AcceptNegotiateNFTModal({
  assetId,
  assetName,
  assetAddress,
  assetLocation,
  ...
  const sendNotification = async (
    ...
    await notificationApi.sendNotification({
      level: "INFO",
      type: ENotificationTypes.RETX_BID_ORDER_ACCEPTED,
      time: parseFloat(timestamp.toString()),
      data: {
        walletAddresses: [infoNegCmd.maker],
    ...
    await notificationApi.sendNotification({
      ...
      data: {
        walletAddresses: listNeg
          .filter((item) => !!item.is_open && item.maker != infoNegCmd.maker)
          .map((item) => {
            return item.maker;
          }),
        nftId: numberNFT.toString(),
  ...
  const accept = async () => {
    try {
      ...
      let output = await web3Contract.nftExchange.methods
        .negCmd_accept(assetAddress, numberNFT, negIndex)
    ...
    <>
      <ModalBase visible={visible} width={500} onCancel={() => {}}>
        <div className="bg-white w-100 p-3 modal-trade--container">
          <h4 className="text-center">{trans[locate].agree_to_sell}</h4>
          <a
            onClick={() => {
              onCancel();
            }}
          ...
          <div className="content">
              ...
              <div className="d-flex align-items-center">
                ...
                {loadingFee ? (
                  ...
                  <span>
                    ...
                    {((fee * 100) / infoNegCmd.price).toFixed(2)}%)
                  </span>
                )}
              </div>
              <div>
            ...
            <div className="confirm">
              ...
              <div className="d-flex gap-3 mt-2">
                <button
                  onClick={onCancel}
                  className="btn btn-orange-light w-50 d-flex justify-content-center gap-3"
                >
                  {trans[locate].cancel}
                </button>
                <button
