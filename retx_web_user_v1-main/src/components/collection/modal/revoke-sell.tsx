...
import { notificationApi } from "@apis/index";
...
export default function RevokeSellNFTModal({
  assetId,
  assetName,
  assetAddress,
  assetLocation,
  ...
  useEffect(() => {
    (async () => {
      await getFee(infoSellCmd.price);
  ...
  const revoke = async () => {
    try {
      ...
      let output = await web3Contract.nftExchange.methods
        .sellCmd_revoke(assetAddress, numberNFT)
        .send({ from: user.walletAddress });
      console.log("🚀 ~ file: revoke-sell.tsx:142 ~ revoke ~ output:", output);
      let { timestamp } = await web3.eth.getBlock(output?.blockNumber);
    ...
    <>
      <ModalBase visible={visible} width={500} onCancel={() => {}}>
        <div className="bg-white w-100 p-3 modal-trade--container">
          <h4 className="text-center">{trans[locate].cancel_sale}</h4>
          <a
            onClick={() => {
              onCancel();
            }}
