...
export default function SubmitNegotiateNFTModal({
  assetName,
  assetAddress,
  assetLocation,
  numberNFT,
  ...
  const validate = () => {
    let errors = {};
    if (price < 1000) {
      errors["price"] = trans[locate].minimum_price_is.replace(':amount', '1000đ');
    }
  ...
  const isApproveStableCoin = async () => {
    try {
      ...
      console.log(
        "🚀 ~ file: submit-neg.tsx:157 ~ isApproveStableCoin ~ allowance:",
        allowance
      );
      setApproved(allowance >= price);
  ...
  const approve = async () => {
    try {
      ...
      await web3Contract.stableCoin.methods
        .approve(infoContract.nftExchange.address, price)
  ...
  const negotiate = async () => {
    try {
      ...
      let value = await web3Contract.nftExchange.methods
        .negCmd_submit(assetAddress, numberNFT, price)
        .send({ from: user.walletAddress });
    ...
    <>
      <ModalBase visible={visible} width={500} onCancel={() => {}}>
        <div className="bg-white w-100 p-3 modal-trade--container">
          <h4 className="text-center">{trans[locate].negotiate}</h4>
          <a
            onClick={() => {
              onCancel();
            }}
          ...
          <div className="content">
            <div className="info">
              ...
              <div>
                ...
                <div>
                  <span className="me-1">{trans[locate].product_order_number}:</span>
                  <span style={{ color: "red" }}>{numberNFT}</span>
                </div>
              </div>
            </div>
            <div className="price">
              <div className="desired-price-input mb-4">
                <div className="head">
              ...
              <div className="desired-price-input">
                <div className="head">
                  <span>{trans[locate].desired_purchase_price}</span>
                ...
                <div className="input">
                  <Input
                    ...
                    placeholder={trans[locate].minimum_price_is.replace(':amount', '1000đ')}
                  />
                  {errors["price"] && (
              ...
              <div className="d-flex align-items-center">
                ...
                {loadingFee ? (
                  ...
                  <span>
                    {fee.toLocaleString('vi-VN')}đ ({((fee * 100) / price).toFixed(2)}
                    %)
                  </span>
                )}
              </div>
