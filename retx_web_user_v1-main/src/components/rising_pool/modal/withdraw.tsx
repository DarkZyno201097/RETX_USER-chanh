...
export default function WithdrawRisingPoolModal({
  assetId,
  assetImage,
  assetName,
  assetLocation,
  ...
  const { balance, web3Contract, infoContract, web3 } = useSelector(contractSelector);
  ...
  const [maxMVNDBalance, setMaxMVNDBalance] = useState<number>(null)
  ...
  useEffect(() => {
    (async () => {
      if (visible && contractPool) {
        ...
        setMaxMVNDBalance(parseFloat(poolBalance?.vnd_balance));

        await isApproveAsset();
    ...
    if (contractPool && web3Contract.stableCoin) {
      const timeOutId = setTimeout(async () => {
        try {
          ...
          // load fee mvnd
          let _fee = await contractPool.methods.getWithdrawFeeRate().call();
          _fee = parseFloat(_fee)
          // console.log("🚀 ~ file: withdraw.tsx:96 ~ timeOutId ~ _fee:", _fee)
          setFee((_fee* _mvndAmount) / 1000);

          await isApproveAsset(assetAmount);
        } catch (e) {
          setFee(0);
          console.log("Error", e);
        } finally {
  ...
  const validate = () => {
    ...
    } else if (assetAmount > investedAmount) {
      errors["assetAmount"] = trans[locate].insufficient_balance;
    }
    if (mvndAmount > maxMVNDBalance) {
      errors["mvndAmount"] = "Vượt quá số dư Pool";
    }
    if (!agreePolicy) {
  ...
  const isApproveAsset = async (amount?: number) => {
    try {
      setLoadingAllowance(true);
      let allowance = await assetContract.methods
        .allowance(user.walletAddress, poolInfo.poolAddress)
        .call();
      allowance = parseInt(allowance);

      let balance = amount ? amount : assetAmount;
  ...
  const approve = async () => {
    try {
      ...
      await assetContract.methods
        .approve(poolInfo.poolAddress, assetAmount)
  ...
  const submit = async () => {
    ...
    if (errors) {
      ...
      let _approved = await isApproveAsset();
      if (_approved) {
        await withdraw();
      } else {
        await approve();
    ...
    <>
      <ModalBase visible={visible} width={500} onCancel={() => {}}>
        <div className="bg-white w-100 p-3 modal-trade--container">
          <h4 className="text-center">Rút vốn</h4>
          <a
            onClick={() => {
              onCancel();
            }}
              ...
              <div className="desired-price-input mb-0">
                <div className="head">
                  ...
                  <span className="d-flex gap-1" style={{ color: "#9EA3AE" }}>
                    ...
                    <span style={{ color: "#111" }}>
                      {investedAmount != null ? (
                        `${investedAmount?.toLocaleString("vi-VN")} Chứng chỉ`
                      ) : (
                ...
                <div className="input">
                  <Input
                    ...
                    disabled={loadingFee || loadingApprove || loadingWithdraw}
              ...
              <div className="desired-price-input">
                <div className="head">
                  ...
                  <span className="d-flex gap-1" style={{ color: "#9EA3AE" }}>
                    ...
                    <span style={{ color: "#111" }}>
                      {(maxMVNDBalance != null) ? (
                        `${maxMVNDBalance?.toLocaleString("vi-VN")}đ`
                      ) : (
                        <Skeleton count={1} style={{ width: 50 }} />
                      )}
                    </span>
            ...
            <div className="confirm">
              ...
              <div className="d-flex gap-3 mt-2">
                ...
                >
                  ...
                  <CircleSpinner
                    loading={loadingWithdraw || (loadingApprove && approved)}
                    size={20}
                  />
                </button>
              </div>
